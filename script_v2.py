...
parser.add_argument("-mv", "--model_version", type=int,
                    help="the version of the model, needed when submitting the job.")

#parser.add_argument("-s", "--s", type=int, help="s.", default=0)
...
def bind():
    print(1)
    #os.system("bash ./docker_deploy.sh --bind -m {} -mid {} -mv {} -gid {} -hid {}".format(work_mode, model_id, model_version, gid, hid))
